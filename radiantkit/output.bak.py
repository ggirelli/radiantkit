...
class OutputType(Enum):
    ...
    def plot(self, **data) -> go.Figure:
        plot_fun: Dict[str, Callable] = dict(
            tiff_findoof=plot.plot_tiff_findoof,
            select_nuclei=lambda *x, **y: None,  # plot.plot_nuclear_selection,
            measure_objects=lambda *x, **y: None,  # plot.plot_nuclear_features,
            radial_population=lambda *x, **y: None  # plot.plot_profiles,
        )
...
class OutputChecker(object):
    ...
    def is_in_folder(self, dpath: str, subname: Optional[str] = None) -> bool:
...
class OutputFinder(object):
    ...
    def search_recursive(
        ...
        if skipHidden:
            subfolder_list = [f for f in subfolder_list if not f.name.startswith(".")]
...
class OutputReader(object):
    ...
    @staticmethod
    def read_tsv(
        ofname: str, dpath: DirectoryPath, subname: str = const.default_subfolder
    ) -> pd.DataFrame:
        return OutputReader.read_csv(ofname, dpath, subname, "\t")

    @staticmethod
    def read_pkl(
        ofname: str, root_path: DirectoryPath, subname: str = const.default_subfolder
    ) -> Dict:
        opath = os.path.join(root_path, ofname)
        if not os.path.isfile(opath):
            opath = os.path.join(root_path, subname, ofname)
    ...
    def read_single_file(
        oflab: OutputFileLabel,
        ofname: str,
        dpath: DirectoryPath,
        subname: str = const.default_subfolder,
    ) -> OutputData:
        if ofname.endswith(".csv"):
            return {oflab: OutputReader.read_csv(ofname, dpath, subname)}
        if ofname.endswith(".tsv"):
            return {oflab: OutputReader.read_tsv(ofname, dpath, subname)}
        elif ofname.endswith(".pkl"):
            return {oflab: OutputReader.read_pkl(ofname, dpath, subname)}
        else:
    ...
    def read(
        ...
        for root_path in path_list:
            ...
            for oflab, ofname in otype.file_labels:
                root_data.update(
                    OutputReader.read_single_file(oflab, ofname, root_path, subname)
                )
    ...
    @staticmethod
    def read_recursive(
        dpath: str, inreg: Pattern, subname: str = const.default_subfolder
...
class OutputPlotter(object):
    _data: Dict[ScriptStub, Dict[BaseName, go.Figure]]

    def __init__(self, output_list: Dict[ScriptStub, Dict[DirectoryPath, OutputData]]):
        super(OutputPlotter, self).__init__()
        self._data = {}
        for script_stub, script_data in output_list.items():
            self._data[script_stub] = {}
            logging.info(f"preparing plot for '{script_stub}' data")
            self._data[script_stub] = OutputType(script_stub).plot(**script_data)
