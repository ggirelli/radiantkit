...
class NuclearSelectionPlotter(object):
    ...
    def __init__(self, raw_data: pd.DataFrame, fit_data: Dict[str, Dict[str, Any]]):
    ...
    def __mk_scatters(self) -> Tuple[go.Scatter, go.Scatter]:
        scatter_selected = go.Scatter(
            ...
            x=self._x[self._passed],
            y=self._y[self._passed],
            mode="markers",
            marker=dict(size=4, opacity=0.5, color="#1f78b4"),
            xaxis="x",
            yaxis="y",
            customdata=np.dstack(
                (
                    self._raw_data[self._passed]["label"],
                    self._raw_data[self._passed]["image"],
                )
            )[0],
        ...
        scatter_filtered = go.Scatter(
            ...
            x=self._x[self._not_passed],
            y=self._y[self._not_passed],
            mode="markers",
            marker=dict(size=4, opacity=0.5, color="#e31a1c"),
            xaxis="x",
            yaxis="y",
            customdata=np.dstack(
                (
                    self._raw_data[self._not_passed]["label"],
                    self._raw_data[self._not_passed]["image"],
                )
            )[0],
            hovertemplate="Size=%{x}<br>Intensity sum=%{y}<br>"
            + "Label=%{customdata[0]}<br>"
    ...
    def __mk_border_lines(self, fig: go.Figure) -> go.Figure:
        fig.add_shape(
            go.layout.Shape(
                ...
                x0=self.size_range[0],
                y0=0,
                x1=self.size_range[0],
                y1=self._y.max(),
            )
        )
        fig.add_shape(
            go.layout.Shape(
                type="line",
                line=dict(dash="dash", color="#969696"),
                x0=self.size_range[1],
                y0=0,
                x1=self.size_range[1],
                y1=self._y.max(),
            )
        )
        fig.add_shape(
        ...
        fig.add_shape(
            go.layout.Shape(
                type="line",
                line=dict(dash="dash", color="#969696"),
                y0=self.isum_range[0],
                x0=0,
                y1=self.isum_range[0],
                x1=self._x.max(),
            )
        )
        fig.add_shape(
            go.layout.Shape(
                type="line",
                line=dict(dash="dash", color="#969696"),
                y0=self.isum_range[1],
                x0=0,
                y1=self.isum_range[1],
                x1=self._x.max(),
            )
        )
        fig.add_shape(
...
class NuclearFeaturePlotter(object):
    ...
    def __init__(self, obj_data: pd.DataFrame, spx_data: Optional[pd.DataFrame]):
        ...
        self._n_rows = (
            self._n_features + len(self._channel_list)
        ) // self.n_grid_cols + 1
    ...
    def __add_precomputed_box(self, spx_channel, ci, root) -> None:
        ...
        self._fig.update_traces(
            q1=spx_root["q1"].values,
            q3=spx_root["q3"].values,
    ...
    def __init_canvas(self) -> None:
        self._fig = make_subplots(
            rows=self._n_rows, cols=self.n_grid_cols, horizontal_spacing=0.2
        )
        self._layout = {}

    def __update_layout(self) -> None:
        self._fig.update_xaxes(tickangle=45)
        self._fig.update_xaxes(ticks="outside", tickwidth=1, ticklen=5, automargin=True)
        self._fig.update_yaxes(ticks="outside", tickwidth=1, ticklen=5, automargin=True)
        self._fig.update_layout(**self._layout)
        self._fig.update_layout(height=400 * self._n_rows, width=1000, autosize=False)
    ...
    def __draw_precomputed_boxes(self) -> None:
        if self._spx_data is not None:
            for ci in range(self.nfeat, self.nfeat + len(self._channel_list)):
                ...
                spx_channel = self._spx_data[self._spx_data["channel"] == channel]
...
class RadialProfilePlotter(object):
    ...
    def plot_channel_profile(
        ...
        for pi in range(len(stype_list)):
            data.extend(
                self.__add_profile_traces(
                    stype_list[pi].value, profiles[stype_list[pi]], raw_data, pal[pi]
                )
            )

        yranges = dict(
            y=(
                np.min([trace["y"].min() for trace in data if "y" == trace["yaxis"]]),
                np.max([trace["y"].max() for trace in data if "y" == trace["yaxis"]]),
            ),
            y2=(
                np.min([trace["y"].min() for trace in data if "y2" == trace["yaxis"]]),
                np.max([trace["y"].max() for trace in data if "y2" == trace["yaxis"]]),
            ),
            y3=(
                np.min([trace["y"].min() for trace in data if "y3" == trace["yaxis"]]),
                np.max([trace["y"].max() for trace in data if "y3" == trace["yaxis"]]),
            ),
        )

        for pi in range(len(stype_list)):
            data.extend(
                self.__add_profile_roots(
        ...
        layout = go.Layout(
            ...
            yaxis=dict(domain=[0.66, 1], range=yranges["y"], title="Intensity (a.u.)"),
            yaxis2=dict(
                domain=[0.33, 0.63],
                range=yranges["y2"],
