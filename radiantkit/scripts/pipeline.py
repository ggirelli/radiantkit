...
def init_parser(subparsers: argparse._SubParsersAction
    ...
    parser.add_argument(
        'workflow', metavar="name", type=str, choices=["yfish"],
        help='''Name of the workflow. Available: yfish''')
    parser.add_argument(
        'root', type=str,
        help='''Path to folder where to set up the workflow.''')

    parser.add_argument(
        '--conda-env', type=str, metavar='STRING', help='''Name of the
        conda environment where radiantkit is installed to.''')
    parser.add_argument(
        '--shell-type', type=str, metavar='STRING',
        help='''Shell type where to run snakemake. One of: bash, zsh.''')

    parser.add_argument(
        '-f', action='store_const', help="""Overwrite existing folder.""",
...
def parse_arguments(args: argparse.Namespace) -> argparse.Namespace:
    ...
    assert not os.path.isdir(args.root) or args.f, (
        "workflow folder already exists")
