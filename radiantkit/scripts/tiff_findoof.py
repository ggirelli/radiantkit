...
''', formatter_class=argparse.RawDescriptionHelpFormatter,
    ...
    advanced.add_argument(
        '--threads', metavar="NUMBER", type=int, default=1,
        help="""Number of threads for parallelization. Default: 1""")
    advanced.add_argument(
        '--intensity-sum', action='store_const', const=True, default=False,
        help="""Use intensity sum instead of gradient magnitude.""")
    advanced.add_argument(
        '--rename', action='store_const', const=True, default=False,
        help="""Rename out-of-focus images by adding the '.old' suffix.""")
    advanced.add_argument('--silent', action='store_const',
                          const=True, default=False, help="""Silent run.""")
...
def plot_profile(args: argparse.Namespace, series_data: pd.DataFrame,
                 path: str) -> None:
    ...
    for profile_data in series_data:
        ...
        mplt.pyplot.plot(
            profile_data['x'], profile_data['y'], linewidth=.5)
    xmax = max(xmax)
    ymax = max(ymax)
    ...
    mplt.pyplot.legend(image_names, bbox_to_anchor=(1.04, 1),
                       loc="upper left", prop={'size': 6})
    mplt.pyplot.subplots_adjust(right=0.75)

    mplt.pyplot.gca().axvline(x=xmax * args.range / 2, ymax=ymax,
                              linestyle="--", color="k")
    mplt.pyplot.gca().axvline(x=xmax - xmax * args.range / 2, ymax=ymax,
                              linestyle="--", color="k")
...
def is_OOF(args: argparse.Namespace, ipath: str,
    ...
    halfrange = img.shape[0]*args.range/2.
    halfstack = img.shape[0]/2.
