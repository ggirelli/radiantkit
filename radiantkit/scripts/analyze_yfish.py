...
def init_parser(subparsers: argparse._SubParsersAction
    ...
    nuclear_selection.add_argument(
        '--k-sigma', type=float, metavar="NUMBER",
        help="""Suffix for output binarized images name.
        Default: 2.5""", default=2.5)
    nuclear_selection.add_argument(
        '--use-all-nuclei', action='store_const',
    ...
    minor.add_argument(
        '--description', type=str, nargs='*', metavar="STRING",
    ...
    advanced.add_argument(
        '--debug', action='store_const', dest='debug_mode',
    ...
    advanced.add_argument(
        '-t', type=int, metavar="NUMBER", dest="threads",
...
def parse_arguments(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.description is not None:
        ...
        args.readable_description = ("\n"+" "*21).join(
            [f"{c} => {v}" for (c, v) in args.description.items()])
    else:
...
def confirm_arguments(args: argparse.Namespace) -> None:
    ...
    with open(path_join(args.output, "analyze_yfish.config.txt"), "w+") as OH:
...
def build_conditions(args: argparse.Namespace) -> List[series.SeriesList]:
    ...
    for condition_name in listdir(args.input):
        ...
        condition = series.SeriesList.from_directory(
        ...
        if args.description is not None:
            if condition_name in args.description:
                ...
                condition.label = condition_name
        else:
            condition.label = condition_name
...
def run(args: argparse.Namespace) -> None:
    ...
    for condition in build_conditions(args):
        ...
        if 1 == args.threads:
            condition.series = [s.extract_particles(
                                    s, [args.ref],
