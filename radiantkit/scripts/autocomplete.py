...
@click.version_option(radiantkit.__version__)
def __main(shell_type: str, regenerate: bool) -> None:
    user_home_path = os.path.expanduser("~")
    autocomplete_path = os.path.join(
        radiantkit.__path__[0], "autocomplete", f".radiant-complete.{shell_type}"
    )
...
def regenerate_autocompletion_files(shell_type: str, autocomplete_path: str) -> None:
    os.system(f"_RADIANT_COMPLETE={shell_type}_source radiant > {autocomplete_path}")
...
def autocomplete_fish(user_home_path: str, autocomplete_path: str) -> None:
    destination_path = os.path.join(
        user_home_path, ".config/fish/completions/radiant.fish"
    )
